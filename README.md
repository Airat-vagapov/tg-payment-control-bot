...
Telegram-бот для контроля ежемесячных платежей в группе: выставляет счет участнику, отслеживает статус оплаты и запускает проверку в дедлайн через очередь задач.

## Что умеет сейчас

- `/setup` — подключает группу и сохраняет базовые настройки (сумма, TZ, дедлайн).
- `/pay` — создает/находит счет за текущий период и показывает кнопки оплаты.
- `/status` — показывает статус счета текущего пользователя.
...
2. Участник вызывает `/pay`.
3. `billings.ensureInvoiceAndSchedule`:
   - определяет период (`YYYY-MM`) в TZ группы,
   - создаёт/находит invoice через `upsert`,
   - ставит due-check job в `pg-boss` на `dueAt`.
4. При оплате mock-провайдер переводит invoice в `paid`.
5. В момент дедлайна job:
